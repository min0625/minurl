...
func TestIsValidOriginalURL(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "c1 control character", rawURL: "https://example.com/a\u0080b", wantErr: true},
		{
			// A raw C1 byte is invalid UTF-8 and decodes to U+FFFD, which is neither
			// IsSpace nor IsControl, so without the RuneError screen it would pass here
			// while its correctly encoded form above is rejected.
			name:    "invalid utf-8 byte",
			rawURL:  "https://example.com/a\x80b",
			wantErr: true,
		},
		{
			// The same screen, reached the other way: encoding/json turns the byte above
			// into this before any resolver sees it, so on the create path a literal
			// U+FFFD is the only trace left of a mangled request.
			name:    "replacement character",
			rawURL:  "https://example.com/a\uFFFDb",
			wantErr: true,
		},
		{
			// url.Parse accepts a literal space anywhere after the authority, so without
			// the whitespace screen this reaches the Location header unencoded.
			name:    "space in path",
			rawURL:  "https://example.com/a b",
			wantErr: true,
		},
		{name: "space in query", rawURL: "https://example.com/?q=a b", wantErr: true},
		{name: "no-break space", rawURL: "https://example.com/a\u00a0b", wantErr: true},
		{
			// The format characters (category Cf) are invisible, not control characters,
			// so unicode.IsControl does not cover them. U+202E reverses the text after
			// it, which turns a path ending in "gnp.exe" into one displayed as
			// "exe.png" wherever the Location header is previewed: the same spoof the
			// userinfo rule exists to stop.
			name:    "right-to-left override",
			rawURL:  "https://example.com/\u202egnp.exe",
			wantErr: true,
		},
		{name: "zero width space", rawURL: "https://example.com/a\u200bb", wantErr: true},
		{name: "byte order mark", rawURL: "https://example.com/a\ufeffb", wantErr: true},
		{name: "soft hyphen", rawURL: "https://ex\u00adample.com/", wantErr: true},
		{
			// Length is not a safety rule and is deliberately unlimited; a very long URL
			// is as safe to redirect to as a short one.
			name:   "very long",
			rawURL: "https://example.com/" + strings.Repeat("a", 64*1024),
		},
	}
