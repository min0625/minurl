...
type ShortURL struct {
	ID          string      `json:"id,omitzero"          doc:"Unique id"                                   maxLength:"12" pattern:"^[123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz]*$" patternDescription:"Base58 characters" nullable:"true"`
...
// unmarshals into the struct.
//
// minLength is load-bearing, not decoration: required:"true" only asserts the key is
// present, and Resolve deliberately returns nil for the empty string, so without
// minLength an empty original_url would be accepted and stored.
//
// There is deliberately no maxLength — see IsValidOriginalURL.
